...
def _load_marketing_posts(
    ...
    *,
    ...
    if status_filter and status_filter != "any" and "status" in cols:
        where.append("status = ?")
...
def _update_marketing_post_status(
    ...
    *,
    ...
    # Only update if the row isn't already published (idempotent)
    cur = conn.execute("SELECT status FROM marketing_posts WHERE id = ? LIMIT 1", (post_id,)).fetchone()
    if cur is None:
        return
    if str(cur[0]) == "published" and new_status == "published":
        return
    ...
    if meta_target:
        ...
        curmeta = _try_json_loads(row[0]) if row is not None else None
        if not isinstance(curmeta, dict):
            curmeta = {}
        curmeta["published_at"] = published_at
        curmeta["published_simulated"] = True
        blob = json.dumps(curmeta, ensure_ascii=False, sort_keys=True)
...
def cmd_publish_marketing(args: argparse.Namespace) -> int:
    ...
    # 1) --status (planned|published|any)
    # 2) --only-planned (legacy)
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        for p in posts:
            ...
            platform = str(p.get("platform", "unknown"))
            track_id = str(p.get("track_id", ""))
            status_before = str(p.get("status", ""))
            ...
            if args.commit:
                # Commit means: mark as published if not already published.
                _update_marketing_post_status(conn, post_id=post_id, new_status="published", published_at=published_at)
            ...
            results.append(
                {
                    ...
                    "permalink": receipt["permalink"],
                    "committed": bool(args.commit),
                    "receipt_written": not (out.exists() and not args.force) or args.force,
                }
            )
